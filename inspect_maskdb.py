...
with open('model/res50-fpn/cityscape/alternate/cache/Cityscape_mask_rcnn1.pkl') as f:
    maskdb = pkl.load(f)

with open('model/res50-fpn/cityscape/alternate/cache/Cityscape_roidb_rcnn1.pkl') as f:
    roidb = pkl.load(f)
...
for roi_rec in roidb:
    ...
    for mask, idx in zip(mask_targets, mask_inds):
        ...
        if (box[3]-box[1])*(box[2]-box[0]) < 10000:
            continue
        rgb = im[box[1]:box[3], box[0]:box[2], :]
        cv2.imwrite('check/{}_{}.jpg'.format(n_img, n_mask), rgb)
