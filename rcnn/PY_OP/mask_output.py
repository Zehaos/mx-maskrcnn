...
class MaskOutputOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        act = in_data[0]
        mask_prob = 1.0 / (1 + mx.nd.exp(-act))
        self.assign(out_data[0], req[0], mask_prob)

    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        assert len(in_data) == 4
        assert len(out_data) == 1
        _, mask_target, mask_weight, label = in_data
        mask_prob = out_data[0]
...
class MaskOutputProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        return ['mask_act', 'mask_target', 'mask_weight', 'label']

    def list_outputs(self):
        return ['output_prob']

    def infer_shape(self, in_shape):
        mask_act_shape = in_shape[0]
        mask_target_shape = in_shape[1]
        mask_weight_shape = in_shape[2]
        label_shape = in_shape[3]

        assert mask_act_shape[0] == mask_target_shape[0], \
            'mask_act_shape[0] != mask_target_shape[0], {} vs {}'.format(mask_act_shape[0], mask_target_shape[0])
        assert mask_act_shape[0] == mask_weight_shape[0]
        assert mask_act_shape[0] == label_shape[0]
        assert mask_target_shape[2] == mask_target_shape[3]

        output_mask_shape = mask_act_shape
